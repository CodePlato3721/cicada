...
function silenceMsToRedemptionFrames(silenceMs) {
...
export class StreamingVad {
  ...
  constructor(model, frameProcessor, resampler) {
    this.model = model;
  ...
  // onSpeechStart：诊断用的可选回调，VAD 概率刚越过阈值、判定"开始说话"的那一帧触发，
  // 用来定位"从开口到 VAD 确认"之间是不是有延迟。
  async feed(stereoBuffer, { onSpeechStart } = {}) {
  ...
  forceEnd() {
    ...
    return msg === Message.SpeechEnd ? audio : null;
  }
