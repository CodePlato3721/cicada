...
// 一个术语词条：translations[lang] 可以是一个字符串，也可以是一个数组——数组时第一项
// 是"官方/规范译法"（用于当这门语言是目标语言时替换成什么），其余项是已知的同音/异写
// 变体（比如 STT 把 "Molly" 识别成"莫莉"而不是官方译名"茉莉"），仅用于扩大匹配面，
// 不会被当成输出结果。
interface TermValue {
...
function loadDictionaries(): Map<string, TermValue[]> {
  const byGame = new Map<string, TermValue[]>(); // gameId -> entries[]
...
function buildAutomatons(dictionariesByGame: Map<string, TermValue[]>): Map<string, Map<string, AutomatonEntry>> {
  const result = new Map<string, Map<string, AutomatonEntry>>(); // game -> Map<lang, { regex, lookup }>
  for (const [game, entries] of dictionariesByGame) {
    const byLang = new Map<string, { surface: string; entry: TermValue }[]>(); // lang -> [{ surface, entry }]
